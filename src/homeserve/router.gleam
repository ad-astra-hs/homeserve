...
pub fn handle_request(
  ...
  case wisp.path_segments(req) {
    ...
    ["assets", "misc", extra] -> serve_extra(req, extra)
...
fn serve_asset_unchecked(req: Request, asset: String, cfg: Config) -> Response {
  ...
  let #(path, mime_type) = case is_gif, animated {
    True, False -> {
      let base = string.drop_end(asset, 4)
      #(
        cfg.paths.assets_directory <> "/" <> base <> "_static.jpg",
        "image/jpeg",
      )
    }
    _, _ -> {
      let ext = get_file_extension(asset)
      #(cfg.paths.assets_directory <> "/" <> asset, extension_to_mime(ext))
    }
  }
