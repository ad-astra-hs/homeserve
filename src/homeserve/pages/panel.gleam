...
/// rendering interface for webcomic panels stored in Mnesia.
import gleam/int
...
/// Renders a panel page, returning the HTTP status code alongside the page.
pub fn render_panel(
  panel_index: Int,
  quirked: Bool,
  animated: Bool,
  ...
  case loader.load_panel(panel_index) {
    ...
    Ok(panel) -> {
      ...
      let next_page_text = case loader.load_panel(panel_index + 1) {
        Ok(next) -> Some(next.meta.title)
        Error(_) -> None
      }
      ...
      case quirks.parse_document(panel.content, quirked) {
        ...
        Error(err) -> {
          ...
          #(500, errors.build_error(500, "Failed to parse page content"))
        }
      }
    }
  }
